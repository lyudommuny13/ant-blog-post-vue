<template>
    <form class="grid gap-3 content-start">
        <div>
            <BaseInput label="Title *" placeholder="Title of the article" v-model="formData.title" :error="errors.title"
                @blur="validateField('title', formData.title, 'Title is required')" />
        </div>
        <div>
            <label class="form-label mt-3">Category *</label>
            <select class="form-select shadow-sm" v-model="formData.categoryId"
        ...
        <div>
            ...
            <input type="file" class="form-control shadow-sm" @change="onThumbnailChange" />
        </div>

        <div>
            <label class="form-label mt-3">Content *</label>
            <textarea v-model="formData.content" class="form-control shadow-sm" rows="8"
                placeholder="Write content here..." :class="{ 'is-invalid': errors.content }"
